...
describe('database layer', () => {
  ...
  test('grant_items table exists', () => {
    const result = db.all<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='grant_items'"
    );
    expect(result).toHaveLength(1);
  });

  test('stacking_rules table exists', () => {
    const result = db.all<{ name: string }>(
      "SELECT name FROM sqlite_master WHERE type='table' AND name='stacking_rules'"
    );
    expect(result).toHaveLength(1);
  });
});
